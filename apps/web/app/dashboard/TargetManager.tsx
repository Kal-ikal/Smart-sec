  ...
  const handleAddTarget = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/targets", {
        ...
        body: JSON.stringify({
          url: formatted,
          program_name: programNameInput.trim() || null,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 relative z-10">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Daftarkan URL target VDP/sandbox yang sah untuk diikutkan dalam pemindaian massal otomatis.
          </p>
        </div>
          ...
          <tbody className="divide-y divide-slate-800/60">
            {targets.map((t) => (
              <tr key={t.id} className="hover:bg-slate-900/50 transition-colors">
                ...
                <td className="py-3 px-3 text-slate-400">{t.program_name || "-"}</td>
                <td className="py-3 px-3">
                  {t.is_authorized ? (
                    ...
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-950/90 text-amber-300 border border-amber-800/80">
                ...
                <td className="py-3 px-3 text-right">
                  <button
                    ...
                    disabled={triggeringJobId === t.id || !t.is_authorized}
                    title={!t.is_authorized ? "Konfirmasikan otorisasi scope pemindaian legal terlebih dahulu" : undefined}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm active:scale-95"
                  >
